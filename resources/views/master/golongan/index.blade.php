      ...
      <div class="col-md-12">
          <div class="card">
              <div class="card-header">
                  ...
                  <!-- <a href="{{ route('mastergolongan.create') }}" class="btn btn-success">
      ...
      <div class="modal-body">
          ...
          <div class="form-group">
            <div class="checkbox col-md-10 col-md-offset-2">
              <label><input type="checkbox" name="Borongan">&nbsp;Borongan?</label>
            </div>
          </div>
          <hr>
          <div class="row">
            ...
            <div class="col-md-4">
              <div class="form-group">
                <label>Uang Lembur Minggu</label>
...
$('button[type="submit"]').on('click', function() {
  ...
  $('button[type="submit"] span').toggleClass('fa fa-spinner fa-spin');

  $('#form-golongan').submit();
});

$('#form-golongan').on('submit', function(event) {
  event.preventDefault(); console.log($(this).serializeArray());
...
$('.modal-golongan input:checkbox').on('change', function() {
  if ($(this).is(':checked')) {
    $('input[name="NominalGroupItemNutuk[]"]').prop('readonly', false);
  } else {
    $('input[name="NominalGroupItemNutuk[]"]').prop('readonly', true);
    $('input[name="NominalGroupItemNutuk[]"]').attr('value', '0');
  }
});
  ...
  else if (action == 'update')
  {
    ...
    $.ajax({
      ...
      if (response[0].Borongan == 1) {
        $('input[name="Borongan"]').prop('checked', true);
        $('input[name="NominalGroupItemNutuk[]"]').prop('readonly', false);
      } else {
        $('input[name="Borongan"]').prop('checked', false);
        $('input[name="NominalGroupItemNutuk[]"]').prop('readonly', true);
      }
      ...
      $.each(response[0].GroupItem, function(index, value) {
        ...
        <tr>
          ...
          <td>
            <input type="number" name="NominalGroupItemNutuk[]" class="form-control group" value="${value.NominalGroupItemNutuk}" step="1" min="0" group="${jumlahGroup}" required>
          </td>
          <td>
...
function addrow(table)
{
  ...
  result = `
    <tr>
      ...
      <td>
        <input type="number" name="NominalGroupItemNutuk[]" class="form-control group" value="0" step="1" min="0" group="${jumlahGroup}" required readonly>
      </td>
      <td>
        <select multiple class="form-control ${jumlahGroup}" name="" group="${jumlahGroup}" required disabled>
        </select>
      </td>
      <td>
        <button type="button" onclick="deleterow(this)" class="btn btn-sm btn-danger">
